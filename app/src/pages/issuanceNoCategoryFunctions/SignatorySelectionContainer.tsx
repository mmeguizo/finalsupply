...
const SignatoriesComponent = ({ signatories, onSignatoriesChange }: ParPageProps) => {
  ...
  // Get signatories from the store (for inspection_officer and recieved_from)
  ...
  // Get users from GraphQL query (for recieved_by)
  const {
    data: usersData,
  ...
  // Update local state when props change (all 3 keys)
  useEffect(() => {
    setSelectedSignatories({
      ...
      metadata: signatories.metadata || {
        inspection_officer: { id: '', position: '', role: '' },
  ...
  const getDropdownOptions = (roleKey: string): UserOption[] => {
    if (roleKey === 'recieved_by') {
      ...
      // Use signatories for inspection_officer and recieved_from
  ...
  // 3 signatory roles for No Category issuance
  const signatoryRoles = [
    { key: 'inspection_officer', label: 'Inspection Officer', designation: 'Inspection Officer' },
