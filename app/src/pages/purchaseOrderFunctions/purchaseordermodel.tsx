...
export default function PurchaseOrderModal({
  ...
  const [formData, setFormData] = React.useState<any>({
    ...
    dateOfConformity: purchaseOrder?.dateOfConformity
      ...
      : dayjs(),
    dateOfDelivery: purchaseOrder?.dateOfDelivery ? dayjs(purchaseOrder.dateOfDelivery) : dayjs(),
    dateOfPayment: purchaseOrder?.dateOfPayment ? dayjs(purchaseOrder.dateOfPayment) : dayjs(),
  ...
  const handleConformityDateChange = (date: Dayjs | null) => {
    setFormData((prev: any) => {
      ...
      const deliveryDate = !isNaN(days) ? computeDeliveryDate(date, days) : date;
      return {
        ...prev,
    ...
    >
      ...
      <DialogContent>
        ...
        <Grid container spacing={2} sx={{ mt: 1 }}>
          ...
          <Grid item xs={12} md={6}>
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                ...
                onChange={(newValue) => handleDateChange(newValue, 'dateOfDelivery')}
                // disabled={purchaseOrder ? true : false}
              />
          ...
          {/* Items Section */}
          <Grid item xs={12}>
            <Typography variant="h6" sx={{ mt: 2, mb: 3, display: 'flex', alignItems: 'center' }}>
              Items
              <Button
                startIcon={<AddIcon />}
                onClick={addItem}
                ...
                disabled={
                  ...
                  !isEditing
                    ...
                    : // In Edit mode, disable if current status is completed (prefer formData.status if present)
                      String(formData.status || purchaseOrder?.status || '').toLowerCase() ===
                      'completed'
                }
              >
                Add Item
