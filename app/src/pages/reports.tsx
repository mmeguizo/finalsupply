...
const IARTableComponent = () => {
  ...
  const [getIARItems, { data: iarItemsData, loading: iarItemsLoading }] =
    useLazyQuery(GET_IAR_ITEMS_BY_IAR_ID);
  ...
  const itemColumns = React.useMemo(
    () => createIARItemColumns(handleOpenPrintModal),
...
const ICSTableComponent = () => {
  ...
  const itemColumns = React.useMemo(
    () => createICSItemColumns(handleOpenPrintModal),
    [handleOpenPrintModal]
  );
...
const RISTableComponent = () => {
  ...
  const handleOpenPrintModal = (item: any) => {
    ...
    const reportTitleString = reportTitleWords
  ...
  const itemColumns = React.useMemo(
    () => createRISItemColumns(handleOpenPrintModal),
    [handleOpenPrintModal]
  );
      ...
      <Paper sx={{ width: '100%' }}>
        <DataGrid
          ...
          slots={{
            toolbar: (props) =>
              CustomToolbarForRIS({
                // Ensure this toolbar is correctly named/imported
                props: {
                  ...props,
...
const PARTableComponent = () => {
  ...
  const itemColumns = React.useMemo(
    () => createPARItemColumns(handleOpenPrintModal),
    [handleOpenPrintModal]
  );
...
export default function ReportsPage() {
  ...
  const renderSelectedTable = () => {
    switch (selectedCategory) {
      ...
      default:
        return (
          <Typography sx={{ mt: 2 }}>Please select a report category to view data.</Typography>
        );
    }
  };
