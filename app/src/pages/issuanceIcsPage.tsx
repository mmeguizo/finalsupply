...
export default function IssuanceIcsPage() {
  ...
  const handleOpenAssignmentModal = (item: any) => {
    ...
    if (poRow) {
      // If item already has icsId, open legacy single-item modal for editing
      if (item.icsId) {
  ...
  const handleCloseMultiAssignModal = () => {
    ...
    setMultiAssignExistingICSItems([]);
  };
  ...
  // Open multi-assign modal from PO row "Assign ICS" button
  ...
  return (
    <PageContainer title="" breadcrumbs={[]} sx={{ overflow: "hidden" }}>
      ...
      {/* ICS Assignment Modal (legacy single-item, for editing existing assignments) */}
      <IcsAssignmentModal
      ...
      {/* Multi-Item ICS Assignment Modal */}
      <MultiIcsAssignmentModal
        ...
        existingICSItems={multiAssignExistingICSItems}
