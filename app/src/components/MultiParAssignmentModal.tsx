...
interface MultiParAssignmentModalProps {
  open: boolean;
  onClose: () => void;
  /** All unassigned items from this PO (items without parId and qty > 0) */
  availableItems: any[];
  /** Pre-selected items (when opening from a specific item click) */
  preSelectedItems?: any[];
  /** PO info for display */
  poNumber: string;
  supplier: string;
  /** All items with existing PAR IDs from this PO (for "add to existing" tab) */
  existingPARItems?: any[];
...
export default function MultiParAssignmentModal({
  open,
  onClose,
  availableItems,
  preSelectedItems,
  ...
  const existingPARGroups = useMemo(() => {
    const groups: Record<
      string,
      { parId: string; receivedBy: string; department: string; items: any[] }
    > = {};
    existingPARItems.forEach((item: any) => {
      if (item.parId) {
        ...
        groups[item.parId].items.push(item);
      }
    });
    return Object.values(groups);
  }, [existingPARItems]);

  /* ---------- Handlers for "New PAR" tab ---------- */
  ...
  const handleAddToExisting = async () => {
    ...
    try {
      const result = await addToExistingPAR({
        variables: {
          input: {
            sourceItemId: addToExistingItem,
            quantity: addToExistingQty,
            existingParId: selectedExistingParId,
          },
        },
      });
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      <DialogTitle sx={{ pb: 1 }}>
        ...
        >
          <Box>
            <Typography variant="h6">Multi-Item PAR Assignment</Typography>
      ...
      <DialogContent dividers>
        ...
        {/* ========== TAB 1: Add to Existing PAR ========== */}
        {tabIndex === 1 && (
          <Stack spacing={2}>
            <Typography variant="subtitle2" fontWeight="bold">
              Select an existing PAR ID to add items to:
            </Typography>

            {/* Existing PAR IDs list */}
            <Stack spacing={1}>
              {existingPARGroups.map((group) => (
                <Card
                  key={group.parId}
                  variant="outlined"
                  sx={{
