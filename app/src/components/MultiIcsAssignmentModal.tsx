...
export default function MultiIcsAssignmentModal({
  ...
  const [addToExistingICS, { loading: addLoading }] = useMutation(ADD_ITEM_TO_EXISTING_ICS, {
    refetchQueries: [{ query: GET_ALL_INSPECTION_ACCEPTANCE_REPORT_FOR_ICS }],
  });
  ...
  const [selectedExistingIcsId, setSelectedExistingIcsId] = useState<string>('');
  ...
  const handleCreateNewICS = async () => {
    ...
    try {
      ...
      setSuccessMessage(`ICS ID ${generatedIcsId} created with ${selectedEntries.length} item(s)!`);
      setSelectedItemIds(new Set());
      setItemQuantities({});
      setSignatoryForm(emptySignatoryForm);
      onAssignmentComplete();
  ...
  const handleSplitAndAssign = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Successfully split "${splitSourceItem?.description || 'item'}" into ${splitRows.length} ICS assignment(s)!`
      );
      setSplitSourceItemId('');
      setSplitRows([]);
      onAssignmentComplete();
    ...
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      ...
      <DialogContent dividers>
        ...
        <Tabs
          ...
          sx={{ mb: 2 }}
        >
          <Tab label="Create New ICS" icon={<AddCircleOutlineIcon />} iconPosition="start" />
          <Tab
          ...
          <Stack spacing={2}>
            ...
            <TableContainer component={Paper} variant="outlined">
              <Table size="small">
                ...
                <TableBody>
                  {availableItems
                    .filter((it: any) => !it.icsId && (it.actualQuantityReceived ?? 0) > 0)
                    .map((item: any) => {
                  ...
                  {availableItems.filter(
                    (it: any) => !it.icsId && (it.actualQuantityReceived ?? 0) > 0
                  ).length === 0 && (
                    <TableRow>
            ...
            {selectedEntries.length > 0 && (
              <Card variant="outlined">
                <CardContent>
                  ...
                  >
                    All selected items will share the same ICS ID and signatory info.
                  </Typography>
        ...
        {tabIndex === 1 && (
          <Stack spacing={2}>
            ...
            <Stack spacing={1}>
              {existingICSGroups.map((group) => (
                <Card
                  ...
                  variant="outlined"
                  sx={{
                    borderColor: selectedExistingIcsId === group.icsId ? 'primary.main' : 'divider',
                ...
                >
                  <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
                    ...
                    >
                      <Chip
                        label={group.icsId}
                        color={selectedExistingIcsId === group.icsId ? 'primary' : 'success'}
                        size="medium"
                        sx={{ fontWeight: 'bold' }}
                      />
            ...
            {existingICSGroups.length === 0 && (
              <Alert severity="info">
                No existing ICS assignments found for this PO. Use the "Create New ICS" tab to
        ...
        {tabIndex === 2 && (
          <Stack spacing={2}>
            <Alert severity="info" sx={{ mb: 1 }}>
              Split one item into <strong>multiple end users</strong>. Each split gets its own ICS
              ID. You can split into any number of pieces (e.g. 1 lot → 6 ICS assignments). All
            ...
            <Autocomplete
              ...
              getOptionLabel={(o: any) =>
                `${o.description || o.itemName} — ${o.actualQuantityReceived} ${o.unit || 'unit(s)'} available (${currencyFormat(o.unitCost)}) [${o.tag}]`
              }
              isOptionEqualToValue={(o: any, v: any) => o.id === v.id}
