...
import { CREATE_SINGLE_ICS_ASSIGNMENT, UPDATE_ICS_ASSIGNMENT } from "../graphql/mutations/inventoryIAR.mutation";
...
interface IcsAssignmentModalProps {
  open: boolean;
  onClose: () => void;
  item: any | null;
...
export default function IcsAssignmentModal({
  open,
  onClose,
  item,
  onAssignmentComplete,
}: IcsAssignmentModalProps) {
  /* ---------- GraphQL ---------- */

  const { data: usersData, loading: usersLoading } = useQuery(GET_ALL_USERS);

  const [createICSAssignment, { loading: createLoading }] = useMutation(
    CREATE_SINGLE_ICS_ASSIGNMENT,
    {
      refetchQueries: [
        { query: GET_ALL_INSPECTION_ACCEPTANCE_REPORT_FOR_ICS },
      ],
    }
  );

  const [updateICSAssignment, { loading: updateLoading }] = useMutation(
    UPDATE_ICS_ASSIGNMENT,
    {
      refetchQueries: [
        { query: GET_ALL_INSPECTION_ACCEPTANCE_REPORT_FOR_ICS },
      ],
  ...
  const existingIcsId = item?.icsId || null;
  const tag = item?.tag || "";
    ...
    if (open && item) {
      ...
      if (item.icsId) {
        setSavedAssignments([{
          ...
          receivedFrom: item.icsReceivedFrom || "",
          receivedFromPosition: item.icsReceivedFromPosition || "",
          receivedBy: item.icsReceivedBy || "",
          receivedByPosition: item.icsReceivedByPosition || "",
        }]);
        setRemainingQty(0);
      }
    }
  ...
  const handleGenerateAndSave = async () => {
    ...
    try {
      const result = await createICSAssignment({
        variables: {
          input: {
            sourceItemId: parseInt(item.id, 10),
            quantity: qty,
      ...
      const { newItem, sourceItem, generatedIcsId } = result.data.createSingleICSAssignment;

      setSavedAssignments((prev) => [
        ...prev,
        {
          id: newItem.id,
          icsId: generatedIcsId,
          quantity: qty,
          department: form.department.trim(),
          receivedFrom: form.receivedFrom.name,
          receivedFromPosition: form.receivedFrom.position || form.receivedFrom.role || "",
      ...
      setSuccessMessage(`ICS ID ${generatedIcsId} created and saved!`);
      resetForm();
      onAssignmentComplete();
      ...
      console.error("createICSAssignment error:", err);
  ...
  const saveEdit = async () => {
    ...
    try {
      await updateICSAssignment({
        variables: {
          input: {
            itemId: parseInt(editingId, 10),
            quantity: qty,
      ...
      console.error("updateICSAssignment error:", err);
  ...
  const totalOriginal = existingIcsId ? savedAssignments[0]?.quantity || originalQty : originalQty;
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 1 }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                ...
                &nbsp;|&nbsp; Tag: {tag ? tag.charAt(0).toUpperCase() + tag.slice(1) : "N/A"}
              </Typography>
            </Box>
                ...
                <CardContent sx={{ py: 1.5, "&:last-child": { pb: 1.5 } }}>
                  {editingId === assignment.id ? (
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, flexWrap: "wrap" }}>
                      ...
                      <Chip label={assignment.icsId} color="success" size="medium" sx={{ fontWeight: "bold", fontSize: "0.95rem" }} />
