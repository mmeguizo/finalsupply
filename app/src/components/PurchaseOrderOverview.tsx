...
export default function PurchaseOrderOverview({
  ...
  const deliveryProgress = totalQuantity > 0 ? (totalReceived / totalQuantity) * 100 : 0;

  const deliveredCount = activeItems.filter((i: any) => i.deliveryStatus === 'delivered').length;
  const pendingCount = activeItems.filter(
    (i: any) => !i.deliveryStatus || i.deliveryStatus === 'pending'
  ).length;
  const partialCount = activeItems.filter((i: any) => i.deliveryStatus === 'partial').length;
  ...
  const statusColor =
    po.status === 'completed' ? 'success' : deliveryProgress >= 50 ? 'warning' : 'default';
  const statusLabel =
    po.status === 'completed'
      ? 'Completed'
      : po.status
    ...
    >
      ...
      <Box sx={{ p: 2, overflowY: 'auto', flex: 1, minHeight: 0 }}>
        ...
        {Object.keys(categoryMap).length > 0 && (
          <Paper variant="outlined" sx={{ p: 2, mb: 2, borderRadius: 2 }}>
            <Typography variant="subtitle2" fontWeight={600} sx={{ mb: 1 }}>
              <CategoryIcon sx={{ fontSize: 18, verticalAlign: 'middle', mr: 0.5 }} />
              Category Breakdown
            </Typography>
            ...
            <Table size="small" stickyHeader>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: 600, textAlign: 'right' }}>Recv</TableCell>
                ...
                {activeItems.map((item: any, idx: number) => {
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <Tooltip title={item.description || item.itemName || '—'}>
                      ...
                      <TableCell>
                        {isComplete ? (
                          ...
                          <LocalShippingIcon color="info" sx={{ fontSize: 18 }} />
                        ) : (
